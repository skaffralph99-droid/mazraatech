...
export default function NewCost() {
  ...
  if (saved) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] px-6 animate-scale-in">
        <div className="w-20 h-20 rounded-full bg-farm-green/20 flex items-center justify-center mb-5 animate-pulse-glow">
        ...
        <div className="flex items-center gap-2 text-farm-dim text-sm mb-8">
          ...
          <span>${parseFloat(amount).toLocaleString()} · {selectedCat?.label} · {selectedLand?.name}</span>
        </div>
  ...
  return (
    <div className="p-4 animate-fade-up">
      <button onClick={() => nav(-1)} className="text-farm-green text-sm font-bold flex items-center gap-1 mb-4 hover:gap-2 transition-all"><ArrowRight size={16} /> رجوع</button>
      ...
      <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            {lands.map(l => (
              <button key={l.id} onClick={() => setLandId(l.id)}
                className={`px-4 py-2.5 rounded-xl text-sm font-bold border transition-all duration-200 active:scale-95 ${landId === l.id ? 'bg-farm-green border-farm-green text-farm-dark shadow-lg shadow-farm-green/20' : 'bg-farm-card border-farm-border text-farm-dim hover:border-farm-green/30'}`}>
                {l.name}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {COST_CATEGORIES.map(c => (
              <button key={c.key} onClick={() => setCategory(c.key)}
                className={`flex items-center gap-2 px-3 py-3 rounded-xl text-sm font-bold border transition-all duration-200 active:scale-95 ${category === c.key ? 'bg-farm-green border-farm-green text-farm-dark shadow-lg shadow-farm-green/20' : 'bg-farm-card border-farm-border text-farm-dim hover:border-farm-green/30'}`}>
        ...
        <div className="animate-fade-up delay-3">
          ...
          <input value={amount} onChange={e => setAmount(e.target.value)} type="number" step="0.5"
        ...
        {/* Preview */}
        {landId && category && amount && parseFloat(amount) > 0 && (
          <div className="card border-farm-green/20 bg-farm-green/5 animate-scale-in">
            <div className="flex items-center justify-between">
              ...
              <p className="text-farm-green text-xl font-black">${parseFloat(amount).toLocaleString()}</p>
            </div>
          </div>
        )}
