...
import { COST_CATEGORIES, cat, cropIcon, money } from '../lib/i18n'
    ...
    <div className="p-4 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="card kpi-blue animate-fade-up delay-1">
          ...
          <p className="text-3xl font-black text-white">{lands.length}</p>
        ...
        <div className="card kpi-red animate-fade-up delay-2">
          ...
          <p className="text-3xl font-black text-white">{money(totalCosts)}</p>
        ...
        <div className="card kpi-green animate-fade-up delay-3">
          ...
          <p className="text-3xl font-black text-white">{money(totalRevenue)}</p>
        ...
        <div className={`card animate-fade-up delay-4 animate-glow ${profit >= 0 ? 'kpi-green' : 'kpi-red'}`}>
          ...
          <p className={`text-3xl font-black ${profit >= 0 ? 'text-gradient' : 'text-red-400'}`}>{money(Math.abs(profit))}</p>
        </div>
      </div>
        ...
        <div className="flex gap-2.5 overflow-x-auto pb-2 -mx-1 px-1">
          {COST_CATEGORIES.slice(0, 6).map((c, i) => (
            <Link key={c.key} to={`/costs/new?cat=${c.key}`}
              className={`animate-scale-in delay-${Math.min(i + 5, 10)} flex-shrink-0 rounded-2xl px-4 py-3 flex flex-col items-center gap-2 min-w-[76px] border hover:-translate-y-1 transition-all duration-300 active:scale-90`}
              style={{ background: `${c.color}15`, borderColor: `${c.color}30` }}>
              <span className="text-2xl">{c.icon}</span>
              <span className="text-[10px] font-bold" style={{ color: c.color }}>{c.short}</span>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="animate-fade-up delay-7">
        ...
        {landProfit.length === 0 ? (
          <div className="card text-center py-8">
            ...
            <Link to="/lands" className="text-farm-green text-sm font-bold mt-2 inline-block">إضافة أرض ←</Link>
          </div>
        ) : (
          <div className="space-y-2.5">
            {landProfit.map((l, i) => (
              <Link key={l.id} to={`/lands/${l.id}`}
                ...
                <div className="w-11 h-11 rounded-xl bg-farm-elevated flex items-center justify-center text-xl shrink-0">{cropIcon(l.current_crop)}</div>
                <div className={`w-1 self-stretch rounded-full shrink-0 ${l.profit > 0 ? 'bg-green-400' : l.profit < 0 ? 'bg-red-400' : 'bg-farm-border'}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-farm-steel font-bold text-sm truncate">{l.name}</p>
                  <p className="text-farm-dim text-[10px]">{[l.current_crop, `${Number(l.size_dunams)} دونم`].filter(Boolean).join(' · ')}</p>
                </div>
                <div className="text-left min-w-[64px] shrink-0">
                  <p className={`text-sm font-black ${l.profit > 0 ? 'text-green-400' : l.profit < 0 ? 'text-red-400' : 'text-farm-dim'}`}>
                    {l.profit !== 0 ? (l.profit > 0 ? '+' : '−') + money(Math.abs(l.profit)).slice(1).replace('$', '') : '—'}
                  </p>
                  {l.cost > 0 && <p className="text-farm-dim text-[9px]">م: {money(l.cost)}</p>}
                </div>
                ...
                <div key={key}>
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-black" style={{ color: info.color }}>{pct.toFixed(0)}%</span>
                      <span className="text-farm-dim text-[10px]">{money(amount)}</span>
                    </div>
                  </div>
              ...
              return (
                <div key={c.id} className="flex items-center gap-3 py-3 first:pt-1 last:pb-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-farm-dim text-[10px]">{info.label} · {format(new Date(c.date), 'dd/MM')}</p>
                  </div>
                  <p className="text-red-400 font-black text-sm shrink-0">{money(c.amount)}</p>
                </div>
              )
            })}
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 animate-fade-up delay-10">
        <Link to="/equipment" className="card kpi-amber flex items-center gap-3 hover:-translate-y-0.5 transition-all duration-200">
          <div className="w-10 h-10 rounded-xl bg-amber-500/15 flex items-center justify-center shrink-0"><Wrench size={18} className="text-amber-400" /></div>
        ...
        <Link to="/workers" className="card kpi-purple flex items-center gap-3 hover:-translate-y-0.5 transition-all duration-200">
          <div className="w-10 h-10 rounded-xl bg-purple-500/15 flex items-center justify-center shrink-0"><Users size={18} className="text-purple-400" /></div>
          <div className="min-w-0">
